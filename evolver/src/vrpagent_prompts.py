    ...
    def user_insight_generation(
        ...
        if insight_type == "initialize":
            if related_candidates:
                print("[WARNING] user_insight: 'initialize' type should not have related_candidates, ignoring them")
            ...
            if len(related_candidates) > 1:
                print(f"[WARNING] user_insight: 'mutate' type expects one candidate, using first of {len(related_candidates)}")
