...
//			System.out.println(str);
	...
	/**
	 * Loads a warm start solution from a .sol file.
	 ...
	 * @return true if solution was loaded successfully, false otherwise
	 */
	public boolean loadWarmStartSolution(String filepath)
	{
		...
		try
		{
			...
			while((line = in.readLine()) != null)
			{
				...
				// Check for Cost line (end of routes)
				if(line.startsWith("Cost"))
					break;

				// Parse route line: "Route #N: node1 node2 node3 ..."
				if(line.startsWith("Route"))
				{
					// Find the colon and get the nodes part
					int colonIndex = line.indexOf(':');
					if(colonIndex == -1)
					{
						System.out.println("[WARM START] ERROR: Invalid route format, missing colon: " + line);
						return false;
					}

					String nodesPart = line.substring(colonIndex + 1).trim();

					// Skip empty routes
					if(nodesPart.isEmpty())
					{
						routeIndex++;
						continue;
					}

					// Split by whitespace to get node IDs
					String[] nodeIds = nodesPart.split("\\s+");
					...
					for(String nodeIdStr : nodeIds)
					{
						...
						// Node IDs in file are 1-indexed, matching node.name
						// solution array is 0-indexed where solution[i] has name = i+1
						int solutionIndex = nodeId - 1;

						if(solutionIndex < 0 || solutionIndex >= solution.length)
						{
							System.out.println("[WARM START] ERROR: Invalid node ID: " + nodeId);
							return false;
						}

						// Add node to route
						f += routes[routeIndex].addNodeEndRoute(solution[solutionIndex]);
					}
			...
			// Set the number of routes used
			numRoutes = routeIndex;

			// Remove any empty routes that may have been created
			removeEmptyRoutes();
		...
		{
			System.out.println("[WARM START] ERROR: Failed to read file: " + filepath);
			return false;
		}
		catch(NumberFormatException e)
		{
			System.out.println("[WARM START] ERROR: Invalid number format in solution file");
			return false;
		}
		finally
		{
